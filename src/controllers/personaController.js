 ...
 */
const createAiPersona = async (req, res, next) => {
  try {
    const { userId } = req.auth;
 ...
 * 페르소나 목록을 조회하는 컨트롤러
 */
const getPersonaList = async (req, res, next) => {
  try {
 ...
 */
const getCommunityPersonaDetails = async (req, res, next) => {
  try {
 ...
 */
const getMyPersonaList = async (req, res, next) => {
  try {
 ...
 */
const getMyPersonaDetails = async (req, res, next) => {
  try {
    const personaId = parseInt(req.params.character_id, 10);
