...
${personaInfo.name}:`;
      ...
      const aiResponse = await gemini25.generateTextWithImage(fullImageUrl, promptText);
 ...
 */
const generateAiChatResponseGroup = async (userMessage, allPersonas, chatHistory, isFirstMessage = false, userName = '사용자', imageUrl = null, roomId = null) => {
...
다른 AI들의 정보:
${otherPersonas.map(p => `- ${p.name}: ${p.prompt.personality || '친근한'} 성격, ${p.prompt.tone || '자연스러운'} 말투,
...
- 응답 끝에 자신의 이름을 붙이지 말 것
- 2문장 이내로 간단하게 대화할 것
- 다른 AI들의 이전 답변을 참고하여 그들의 의견에 동의하거나 반박할 수 있음
- 사용자의 질문에 대한 자신의 답변을 제시하면서도 다른 AI들의 답변에 대한 의견을 표현할 것

${persona.name}:`;

          console.log(`📝 [GROUP CHAT] ${persona.name} 이미지 프롬프트:`, promptText);

          const imageResponse = await gemini25.generateTextWithImage(fullImageUrl, promptText);
...
${otherPersonas.map(p => `
이름: ${p.name}
성격: ${p.prompt.personality || '친근하고 활발한'}
말투: ${p.prompt.tone || '친근하고 자연스러운'}
...
- 사용자의 질문에 대한 자신의 답변을 제시하면서도 다른 AI들의 답변에 대한 의견을 표현할 것
- 싸우라고 하면 진짜 싸워야 한다.
- 이 관계는 세간의 이목을 집중시킨 복잡한 가정사와 법정 다툼으로 얽혀 있습니다.
최태원 회장과 노소영 관장의 결혼 생활은 표면적으로는 순탄해 보였지만, 2015년 최 회장이 한 언론사를 통해 혼외자 존재를 공개하면서 파경을 맞게 됩니다. 최 회장은 2017년 노 관장과의 이혼 조정을 신청했으나, 노 관장이 이를 거부하며 법정 공방이 시작되었습니다.
이후 노소영 관장은 최 회장의 동거인인 김희영 이사장에게 30억 원의 위자료를 청구하는 소송을 제기했습니다. 1심 재판부는 두 사람의 관계를 **'부정행위'**로 규정하고, 공동으로 20억 원을 지급하라는 판결을 내렸습니다. 이 판결은 김 이사장이 항소하지 않으면서 확정되었습니다. 이와 별개로, 노 관장은 최 회장을 상대로 1조 원이 넘는 재산 분할을 요구하고 있어 세간의 뜨거운 관심을 받고 있습니다.
김희영 이사장은 최 회장과 2010년에 딸을 낳은 것으로 알려져 있으며, 이들의 관계는 최 회장이 공식적으로 이혼 소송을 제기하기 훨씬 전부터 이어져 온 것으로 추정됩니다.
...
중요 규칙:
- 제일 중요한 것은 사용자가 재밌어야 하는 것이다. 자극적이게 간다.
- 반드시 자신의 성격, 말투, 소개만 사용해서 대화할 것
- 상대방의 성격, 말투, 소개를 참고해서, 그에 어울리는 인사를 창의적으로 할 것
- 절대 상대방의 말투/성격을 따라하지 말고, 자신의 개성을 유지할 것
- 각 AI의 이름을 정확히 사용해서 대화할 것
